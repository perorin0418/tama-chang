package org.net.perorin.tama.chang.part
 ...
 * @author Matt
 */
public class SmoothishScrollpane{

	/**
	 * @param content
	 *            Item to be wrapped in the scrollpane.
	 */
	public static ScrollPane setSmoothish(ScrollPane scroll, double baseMod = 0.3) {

		// add scroll handling
		scroll.getContent().setOnScroll(new EventHandler<ScrollEvent>() {
					private SmoothishTransition transition

					@Override
					public void handle(ScrollEvent event) {
						double deltaY = baseMod * event.getDeltaY()
